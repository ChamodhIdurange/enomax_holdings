...
if($repid != 0){
    $sql="SELECT `tbl_customer_order`.`idtbl_customer_order` as 'orderid', `tbl_customer_order`.`date`, `tbl_customer_order`.`status`, `tbl_customer_order`.`nettotal`, `tbl_customer_order`.`remark`, `tbl_customer`.`customer` as 'customername', `tbl_area`.`area`, `tbl_customer`.`address` FROM `tbl_customer_order` LEFT JOIN `tbl_area` ON `tbl_area`.`idtbl_area`=`tbl_customer_order`.`tbl_area_idtbl_area` LEFT JOIN `tbl_customer` ON `tbl_customer`.`idtbl_customer`=`tbl_customer_order`.`tbl_customer_idtbl_customer` WHERE `tbl_customer_order`.`status`=1 AND `tbl_customer_order`.`tbl_employee_idtbl_employee`='$repid'";
}

if($salesmanagerid != 0){
    $sql="SELECT `tbl_customer_order`.`idtbl_customer_order` as 'orderid', `tbl_customer_order`.`date`, `tbl_customer_order`.`status`, `tbl_customer_order`.`nettotal`, `tbl_customer_order`.`remark`, `tbl_customer`.`customer`  as 'customername', `tbl_area`.`area`, `tbl_customer`.`address` FROM `tbl_customer_order` LEFT JOIN `tbl_area` ON `tbl_area`.`idtbl_area`=`tbl_customer_order`.`tbl_area_idtbl_area` LEFT JOIN `tbl_employee` ON `tbl_employee`.`idtbl_employee`=`tbl_customer_order`.`tbl_employee_idtbl_employee` LEFT JOIN `tbl_sales_manager` ON `tbl_sales_manager`.`idtbl_sales_manager`=`tbl_employee`.`tbl_sales_manager_idtbl_sales_manager` LEFT JOIN `tbl_customer` ON `tbl_customer`.`idtbl_customer`=`tbl_customer_order`.`tbl_customer_idtbl_customer` WHERE `tbl_customer_order`.`status`=1 AND `tbl_sales_manager`.`idtbl_sales_manager`='$salesmanagerid'";
}
