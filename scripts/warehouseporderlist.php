...
$columns = array(
	...
	array( 'db' => '`u`.`idtbl_warehouse`', 'dt' => 'idtbl_warehouse', 'field' => 'idtbl_warehouse' ),
...
$joinQuery = "FROM `tbl_warehouse` AS `u` LEFT JOIN `tbl_porder_otherinfo` AS `ua` ON (`ua`.`porderid` = `u`.`tbl_porder_idtbl_porder`) LEFT JOIN `tbl_area` AS `ub` ON (`ub`.`idtbl_area` = `ua`.`areaid`) LEFT JOIN `tbl_customer` AS `uc` ON (`uc`.`idtbl_customer` = `ua`.`customerid`) LEFT JOIN `tbl_employee` AS `ud` ON (`ud`.`idtbl_employee` = `ua`.`repid`)";

$extraWhere = "`u`.`confirmstatus`=1 AND `u`.`status`=1 AND `u`.`potype`=1";
