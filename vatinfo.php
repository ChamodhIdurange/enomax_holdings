...
if ($result->num_rows > 0) {
    $vatRow = $result->fetch_assoc();
    $currentVat = $vatRow['vat_rate'];
}
            ...
            <div class="container-fluid mt-2 p-0 p-2">
                <div class="card mb-5">
                    <div class="card-body p-0 p-2">
                        <div class="row">
                            <div class="col-3">
                                <form action="process/vatprocess.php" method="post" autocomplete="off">
                                    <div class="form-group">
                                        <label class="small font-weight-bold text-dark">VAT Percentage (%)*</label>
                                        <input type="number" step="0.01" class="form-control form-control-sm"
                                            name="vat" id="vat" required
                                            value="<?php echo $currentVat; ?>">
                                    </div>
                            ...
                            <div class="col-9">
                                <table class="table table-bordered table-striped table-sm nowrap" id="dataTable">
                                    ...
                                    <tbody>
                                        <?php
                                        // Re-query to display in table
                                        $sql2 = "SELECT * FROM tbl_vat";
                                        $result2 = $conn->query($sql2);

                                        if ($result2->num_rows > 0) {
                                            while ($row = $result2->fetch_assoc()) { ?>
                                                <tr>
                                                    <td><?php echo $row['idtbl_vat'] ?></td>
                                                    <td><?php echo $row['vat_rate'] ?></td>
                                                    <td><?php echo ($row['status'] == 1) ? 'Active' : 'Inactive'; ?></td>
                                                    <td><?php echo isset($row['insertdatetime']) ? $row['insertdatetime'] : '-'; ?></td>
                                                </tr>
                                            <?php }
                                        } else { ?>
                                            <tr>
                                                <td colspan="4" class="text-center">No VAT rate configured</td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <button class="btn btn-outline-primary btn-sm btnEdit <?php if ($editcheck == 0) {
                                                                                                                    echo 'd-none';
                                                                                                                } ?>" id="<?php echo $row['idtbl_vat_info'] ?>"><i data-feather="edit-2"></i></button>
                                                        <?php if ($row['status'] == 1) { ?>
                                                            <a href="process/statusvatinfo.php?record=<?php echo $row['idtbl_vat_info'] ?>&type=2" onclick="return confirm('Are you sure you want to deactive this?');" target="_self" class="btn btn-outline-success btn-sm <?php if ($statuscheck == 0) {
                                                                                                                                                                                                                                                                                    echo 'd-none';
                                                                                                                                                                                                                                                                                } ?>"><i data-feather="check"></i></a>
                                                        <?php } else { ?>
                                                            <a href="process/statusvatinfo.php?record=<?php echo $row['idtbl_vat_info'] ?>&type=1" onclick="return confirm('Are you sure you want to active this?');" target="_self" class="btn btn-outline-warning btn-sm <?php if ($statuscheck == 0) {
                                                                                                                                                                                                                                                                                echo 'd-none';
                                                                                                                                                                                                                                                                            } ?>"><i data-feather="x-square"></i></a>
                                                        <?php } ?>
                                                        <a href="process/statusvatinfo.php?record=<?php echo $row['idtbl_vat_info'] ?>&type=3" onclick="return confirm('Are you sure you want to remove this?');" target="_self" class="btn btn-outline-danger btn-sm <?php if ($deletecheck == 0) {
                                                                                                                                                                                                                                                                            echo 'd-none';
