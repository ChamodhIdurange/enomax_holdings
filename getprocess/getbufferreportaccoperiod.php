...
$sqlcustomer="SELECT `tbl_customer`.`idtbl_customer`, `tbl_customer`.`customer`, `tbl_customer_stock`.`fullqty` FROM `tbl_customer` LEFT JOIN `tbl_customer_stock` ON `tbl_customer_stock`.`tbl_customer_idtbl_customer`=`tbl_customer`.`idtbl_customer` WHERE `tbl_customer`.`status`=1 AND `tbl_customer_stock`.`status`=1";
...
<table class="table table-striped table-bordered table-sm" id="tableoutstanding">
    ...
    <tbody>
        ...
        ?>
        <tr>
            <td><?php echo $rowcustomer['customer']; ?></td>
            <?php 
