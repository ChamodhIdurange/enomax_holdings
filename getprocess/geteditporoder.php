...
$sql = "SELECT *, `tbl_area`.`idtbl_area`, `tbl_locations`.`idtbl_locations`, `tbl_porder_otherinfo`.`idtbl_porder_otherinfo` FROM `tbl_porder` LEFT JOIN `tbl_porder_otherinfo` ON (`tbl_porder_otherinfo`.`idtbl_porder_otherinfo` = `tbl_porder`.`idtbl_porder`) LEFT JOIN `tbl_customer` ON (`tbl_customer`.`idtbl_customer` = `tbl_porder_otherinfo`.`customerid`) LEFT JOIN `tbl_area` ON (`tbl_area`.`idtbl_area` = `tbl_porder_otherinfo`.`areaid`) LEFT JOIN `tbl_employee` ON (`tbl_employee`.`idtbl_employee` = `tbl_porder_otherinfo`.`repid`) LEFT JOIN `tbl_locations` ON (`tbl_locations`.`idtbl_locations` = `tbl_porder`.`tbl_locations_idtbl_locations`) WHERE `idtbl_porder`= '$recordID'";
